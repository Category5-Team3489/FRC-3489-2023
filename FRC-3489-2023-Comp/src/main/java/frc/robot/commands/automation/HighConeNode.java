...
public class HighConeNode extends CommandBase {
    private PIDController strafeController = new PIDController(0.18, 0, 0); // 0.12
    ...
    public void execute() {
        ...
        if (!Double.isNaN(targetX)) {
            yMetersPerSecond = -strafeController.calculate(targetX, -3.64); //-3.9
        ...
        if (!Double.isNaN(targetY)) {
            xMetersPerSecond = -distanceController.calculate(targetY, 21.49); // -6.6
